...
export function WhyHeritageSection() {
  return (
    <section className="w-full flex justify-center py-20 px-4 md:px-10 bg-background relative">
        ...
        <Container className="w-full flex flex-col gap-14">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-border/30 pb-8">
                <div className="flex flex-col gap-4 max-w-[650px]">
                    <h2 className="text-primary text-4xl md:text-5xl font-display font-bold tracking-tight">Why Heritage as CSR?</h2>
                    <p className="text-muted-foreground font-body text-xl italic">Investing in conservation offers unique advantages beyond traditional CSR avenues, creating lasting assets and community pride.</p>
                </div>
                <Link href="/services" className="text-accent font-bold font-sans flex items-center gap-2 hover:gap-3 transition-all hover:text-primary uppercase tracking-wider text-sm mb-2">
                ...
                {benefits.map(benefit => (
                    <div key={benefit.title} className="group flex flex-col gap-5 rounded-lg border border-border bg-card p-8 hover:border-accent hover:shadow-xl hover:shadow-border/20 transition-all duration-500 relative overflow-hidden">
                        ...
                        <div className="flex flex-col gap-3">
                            <h3 className="text-primary text-2xl font-display font-bold">{benefit.title}</h3>
